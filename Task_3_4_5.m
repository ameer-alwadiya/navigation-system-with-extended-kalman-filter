...
% detection of AoA fault

y_nominal = Innov_AoA_nominal; % measurement data
y_nominal(1:100) = 0;

y = Innov_AoA_fault; % measurement data
y(1:100) = 0;
% y = y_nominal(100:N);

theta_0 = [mean(y_nominal)];
sigma_0 = [std(y_nominal)];

% Thresholds for positive and negative tests
threshold_pos = [98000]; % Example thresholds for positive tests for each measurement
threshold_neg = [-98000]; % Example thresholds for negative tests for each measurement
...
for m = 1:size(y, 2)
    for k = 1:size(y, 1)-1
        g_pos(k+1, m) = max(0, g_pos(k, m) + s(k, m))-0.2; % positive test
        g_neg(k+1, m) = min(0, g_neg(k, m) + s(k, m))+0.2; % negative test
